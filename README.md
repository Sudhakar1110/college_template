...
| 6 | **WhatsApp / SMS Notifications** | Requires third-party API setup (MessageBird/Twilio). **Script + DocType are ready** — `send_whatsapp_notification` API + `Communication Log` DocType exist. Just needs: `pip install messagebird` on server + API key in site config (`bench --site your-site set-config messagebird_api_key "your_key"`). |
